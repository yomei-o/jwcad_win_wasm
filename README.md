...
`sh tools/check.sh` が **1,267 項目**を通し、そのうえで同梱の図形 341 個・
図面 170 枚を読んで書き戻し、**壊れたファイル 95 万通り**を読ませ、**コマンドを手当たり次第に
叩いて**（`tests/cmdfuzz_test.c`）落ちないことを確かめ、
**同梱の図面 16 枚をネイティブと WASM の両方で描いて 1 画素ずつ**
突き合わせます。
...
`sh tools/asanseeds.sh` はさらに広く、**壊し方の種を 20 通り**変えて
同じサニタイザに通します（1 通り 39 万件、全部で 787 万件）。何時間も
かかるので `check.sh` にも `asan.sh` にも入れていません。読み手を
いじったときの、いちばん外側の網です。
1 本だけ見たいときは `sh tools/asanone.sh <試験名>`。

**乱打だけでは届かないものもあります。**円の環の置き場が溢れるのは
半径 5,792〜8,189 という 1.4 倍の帯でだけで、39 万件×198 ファイルを
ASan で通しても報告ゼロでした。**定義域を端から端まで歩く試験**の
ほうが早く、`tests/bigcirc_test.c` が半径 1〜8,200 を全部通します
（RESUME.md「紙のミリを画素に直すところ」）。
...
sh tools/asanseeds.sh    # 同じものを壊し方 20 通りで（数時間）
```
...
検査は全部で **1,267 項目**あり（このほかに、同梱のファイルを広く読む
ものが増えました —— 図形 341 個の読みと書き戻し、図面 170 枚の読みと
書き戻し、それに**壊れたファイル 95 万通り**を読ませて落ちないか、
**コマンドを手当たり次第に 144 万手叩いて**落ちないか・数が壊れないか・
書いて読み戻してまた書いたら同じバイトか）、その多くは**原典に同じ操作をさせて保存させた
...
**仮点の形**（十字・3×3・1 画素の描き分け）と**レイヤの印の左右**
（左 9 画素が「文字でないものがある」、右 8 画素が「文字がある」）も
入れて、さらに**表示のみレイヤの灰色を先に置く**ようにして、
合計 **804 画素**、**15 枚のうち 5 枚（`Test1`・`Test5`・
`円地図`・`Ａマンション25d`・`Ａマンション立面例`）は 1 画素も
違いません**。残るのは線の位置が 1 画素ずれるものです。

同梱 15 枚ぶんは `sh tools/refshots.sh && sh tools/scoreall.sh`
（基準画像は**前面で**撮る必要があります。背景の `PrintWindow` では
作図領域が当てになりません）。いまは画布に対して 0.000%〜0.052%、
5 枚が 0 画素、悪いのが `Ａマンション平面例` の 392 画素（0.052%）。
15 枚の合計で **804 画素**です（円弧の端の向きを画素で決めるように
した分が 1 画素、真円の環を GDI の `Ellipse` ではなく `Arc` 2 本から
取り直した分が 57 画素、点線が自分の端をはみ出していたのを直した分が
72 画素、部分円弧の環も同じように取り直した分が 17 画素、
円弧の終わりの画素を塗らないようにした分が 13 画素）。

図面のほうは、**字形の入る矩形を別勘定**にしてこの数字です
（`tests/shot.exe` が図面から矩形を書き出し、`tools/cmp.py` がそれを外します）。
**その矩形は窓の 14.3% を覆います**（`Test6` は 35%、いちばん狭いもので
5.6%）。つまり **804 は窓の 85.7% についての数字**です。矩形は文字の枠
ぴったりではなく 1 文字ぶんの高さ＋4 画素の余白を足していて、その余白を
倍にすると 814 は 313 になりました（その測定は 804 になる前のものです） —— **覆いの取り方という判断込みの数**
なので、自分自身との比較として使っています（RESUME.md「採点は窓の
85.7% を見ています」）。
...
それは移植が環境に依存した印になります。いまのところ**差は 0 画素**
——空の枠だけでなく、**`Test1.jww` と `Test7.jww` を開いた絵でも
1 画素も違いません**（`tests/wasm_check.js --open`）。
...
2. ~~**`.jww` を読む。**~~ 済み。同梱 15 枚すべて末尾ぴったり。
   書き戻しも、図形（`.jws`）も、DXF・SFC・JWC・座標ファイルも。
3. ~~**描画。**~~ 済み。線・円弧・点・ソリッド・文字。用紙の合わせ方も
   原典の割り切りどおりです。15 枚の残りは **804 画素**、うち 5 枚は
   1 画素も違いません。
4. ~~**マウスとコマンド。**~~ 主なものは済み。クロックメニューも。
   手つかずは AUTO モード・外部変形・2.5D・日影図・天空図。
5. ~~**WASM。**~~ 済み。ネイティブと**同じ画素**です（16 枚とも 0 差）。

**いま残っているのは、円弧の環をどこで切るかの詰め**（`日影図` の円 1 つ
で 223 画素、`Ａマンション平面例` の端の切り方で 250 画素ほど）と、
**まだ駆動できていないコマンド**です。RESUME.md の冒頭に一覧があります。
