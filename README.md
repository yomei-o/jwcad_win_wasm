...
手元の **`.jww`・`.dxf`・`.sfc`・`.jwc` を開けて**、**`.jww`・`.dxf`・
`.sfc`・`.jwc` で保存できます**（どれも手元だけで、どこにも送られません）。
...
曲線・ハッチ・**包絡処理**・属性取得・属性変更・範囲選択・複写・移動・
図形（ブロック）、それに**線属性**と**書込み文字種変更**のダイアログ。
`sh tools/check.sh` が 900 項目を通します。
