...
合計 **814 画素**、**15 枚のうち 5 枚（`Test1`・`Test5`・
...
15 枚の合計で **814 画素**です（円弧の端の向きを画素で決めるように
...
5.6%）。つまり **814 は窓の 85.7% についての数字**です。矩形は文字の枠
ぴったりではなく 1 文字ぶんの高さ＋4 画素の余白を足していて、その余白を
倍にすると 814 は 313 になります —— **覆いの取り方という判断込みの数**
...
3. ~~**描画。**~~ 済み。線・円弧・点・ソリッド・文字。用紙の合わせ方も
   原典の割り切りどおりです。15 枚の残りは **814 画素**、うち 5 枚は
   1 画素も違いません。
