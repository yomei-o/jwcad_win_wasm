...
| 次 | **文字**。あとは用紙の合わせ方の 0.1% と、図面ごとの色表 |

## 次にやること

1. **文字。** いま図面の不一致 2.38% はほぼ全部これです。`CDataMoji` は
   4 つの座標（始点と終点）＋ 書体名 ＋ 本文を持っています。字形は
   フォント次第なので一致しませんが、**位置と大きさと色は合わせられます**。
   原典は `CreateFontIndirectW` ＋ `ExtTextOutW`。
2. **用紙の合わせ方の残り 0.1%。** 原典は 1.63125 px/mm、素直に
