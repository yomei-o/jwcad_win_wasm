...
| 次 | 線種の刻みの位相、レイヤバーの印、ステータス行の表示、拡大縮小と移動 |

## 次にやること

1. **線種の刻みの位相。** レジストリの 32 ビットパターンをそのまま
   1 ビット 1 画素で送ると周期 32 になりますが、**原典は 32.28** です
   （Test1 の `道路中心線`、一点鎖2 を 26 本ぶん測った値）。
   `Pitch06` は 1 なので、倍率でも何でもない何かが 0.9% ぶん効いています。
   図面の不一致 6,795 画素のうち半分ほどがこれです。
2. **レイヤバーの印とステータス行。** 図面を開くと原典は
   「どのレイヤに要素があるか」を升目の赤枠で示し、ステータス行に
   `A-2 S=1/100 [0-A]#lv4` を出します。いまは起動直後のままです。
