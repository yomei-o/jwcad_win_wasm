...
| 次 | **枠を描く C を書く**。`src/fb.*`（フレームバッファ）→ `src/ui.*`（枠）→ 画素比較 |

## 明日ここから

**枠（メニュー・ツールバー・ステータスバー）から**、という方針で合意済み。
`docs/ref_start.png`（クライアント領域 1264×741、Direct2D オフ）が正解です。

順番はこう考えています。

1. `tools/mkres.py` —— `.rsrc` のツールバー帯（4bpp）とレイアウトを C の配列に焼く。
   生成物は `src/gen/` に置いて **.gitignore**（原典由来なので）。
2. `src/fb.c/h` —— 32bit のフレームバッファと、矩形塗り・3D 枠・4bpp 転送。
3. `src/ui.c/h` —— 枠を描く。まず背景・画布の窪み枠・区切り線・ステータスバー、
   次にボタン。
4. `tests/frame.c` —— PNG に出す。`tools/cmp.py` で `docs/ref_start.png` と
   1 画素ずつ比べて不一致数を出す。**この数字を減らしていくのが作業**です。

`tools/cmp.py` はまだありません。最初に書くとよいです。
