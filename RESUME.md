...
| 次 | `.jww` の読み取りを C に移し、線・円弧・文字を画布に描く |

## 次にやること

1. **`tools/jww.py` を C に移す**（`src/jww.c`）。Python 版がそのまま仕様書です。
   ネイティブと WASM で同じものを使うので、`fopen` ではなくメモリ上の
   バイト列を受け取る形にすること。
2. **描く。** `CJw_winView::OnDraw` と `CData*` の描画を追う。線色・線種・線幅は
   ヘッダの色表と線種表（英語名つきで読めています）から。
   まず `docs/ref_test1.png`（Test1.jww を開いた画面）と突き合わせる。
   **文字は別勘定**。`docs/textareas.txt` に画布を足すことになります。
3. **座標変換。** 画布は 1108x686。用紙の寸法とグループの縮尺から
   どう写すかは `CGamenJoken`（`CJw_winView` の第 2 基底）にあるはず。
